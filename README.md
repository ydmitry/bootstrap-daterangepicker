...
`showCalendarsOnOpen`: (boolean) When ranges is enabled show calendars at once with ranges

`rangeHiddenElement`: (string) Selector of hidden element to save range in implicit way (for instance: "this month")

`hideCalendarsOnSelect`: (boolean) Close calendars when select from date and to date
